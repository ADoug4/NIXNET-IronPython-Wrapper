...
# use byref() for single values and pointer() for arrays
# for each error indicate the function the error came from
...
class nixnet:
    ...
    def create_frame_input_session(self, intf_port, intf_database=":memory:", intf_cluster='', intf_frames=''):
        ...
        try:
            ...
            m_SessionRef = c_uint32(22)
            m_SessionRef_ptr = byref(m_SessionRef)
            # print(m_SessionRef_ptr.contents)
            l_Status = self.XNET_dll.nxCreateSession(
                ...
                nxMode,  # Mode
                m_SessionRef_ptr)  # ctypes.c_void_p

            print(type(m_SessionRef))
            print(m_SessionRef)
            ...
            if status < 0:
                raise Exception('XNET Session Error in create_frame_input_session():  {}'.format(status))

            print('session :', self.session_ptr.value)
            return status
    ...
    def read_bytes(self, number_of_bytes, timeout=0):
        ...
        try:
            ...
            NumberOfBytesReturned_ptr = byref(NumberOfBytesReturned)

            print('session :', self.session_ptr.value)
            status = self.XNET_dll.nxReadFrame(self.session_ptr, Buffer, SizeOfBuffer, nxTimeout_None,
                                               NumberOfBytesReturned_ptr)
    ...
    def set_property_int64(self, propertyID, propertyValue):
        try:
            ...
            status = self.XNET_dll.nxSetProperty(self.session_ptr, property_id, c_uint32(8), byref(property_value))
            if status < 0:
                raise Exception('XNET Session Error :  {}'.format(status))
